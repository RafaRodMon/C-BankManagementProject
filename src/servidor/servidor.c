		...
		if (nuevo_socket != INVALID_SOCKET) {
			...
			while (recv(nuevo_socket, (char*)&msg, sizeof(msg), 0) > 0) {
				...
				case OP_LOGIN:
					...
					const char* sql_login = "SELECT C.nombreCuenta FROM Cliente Cl "
							"INNER JOIN Cuenta C ON Cl.id_cliente = C.id_cliente "
							"WHERE Cl.nombre = ? AND Cl.contrasenya = ?;";
					sqlite3_stmt *stmt;
					int encontrado = 0;

					if (sqlite3_prepare_v2(db, sql_login, -1, &stmt, NULL) == SQLITE_OK) {
						sqlite3_bind_text(stmt, 1, usuario, -1, SQLITE_STATIC);
						sqlite3_bind_text(stmt, 2, contrasenya, -1, SQLITE_STATIC);

						if (sqlite3_step(stmt) == SQLITE_ROW) {
							encontrado = 1;
							snprintf(msg.data, sizeof(msg.data), "%s", (const char*)sqlite3_column_text(stmt, 0));
						}
						sqlite3_finalize(stmt);
					}

					if (encontrado) {
						registrar_log(cfg.log_ruta, "LOGIN: Acceso correcto. Cuenta enviada.");
					} else {
						snprintf(msg.data, sizeof(msg.data), "Error: Credenciales incorrectas.");
				...
				case OP_HISTORIAL:
					registrar_log(cfg.log_ruta, "OPERACIÓN: Consulta de Historial Único.");
					{
						const char* sql = "SELECT tipo, importe, cuenta_ordenante, cuenta_beneficiaria, fecha "
								"FROM Movimiento "
								"WHERE cuenta_ordenante = ? OR cuenta_beneficiaria = ? "
								"ORDER BY fecha DESC LIMIT 5;";

						sqlite3_stmt *stmt;
						char historial_acumulado[sizeof(msg.data)] = {0};
						int vacio = 1;

						if (sqlite3_prepare_v2(db, sql, -1, &stmt, NULL) == SQLITE_OK) {
							sqlite3_bind_text(stmt, 1, msg.data, -1, SQLITE_STATIC);
							...
							while (sqlite3_step(stmt) == SQLITE_ROW) {
								...
								double importe = sqlite3_column_double(stmt, 1);
								const char* ordenante = (const char*)sqlite3_column_text(stmt, 2);
								const char* beneficiaria = (const char*)sqlite3_column_text(stmt, 3);
								const char* fecha = (const char*)sqlite3_column_text(stmt, 4);

								snprintf(linea, sizeof(linea), "[%s] %s: %.2f EUR (%s -> %s)\n",
										fecha, tipo, importe, ordenante, beneficiaria);

								if (strlen(historial_acumulado) + strlen(linea) < sizeof(historial_acumulado) - 1) {
									strcat(historial_acumulado, linea);
								} else {
									break;
								}
							}
						...
						if (vacio) {
							snprintf(msg.data, sizeof(msg.data), "La cuenta %s no registra movimientos.", msg.data);
						} else {
							strncpy(msg.data, historial_acumulado, sizeof(msg.data) - 1);
						}
					}
					break;
